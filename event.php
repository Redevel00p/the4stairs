...
$upcoming_special_events = [];
$schedules_this_week = [];
$past_schedules = [];

if (isset($conn) && !$conn->connect_error && $db_selected) {
    // 1. Ambil event spesial mendatang (is_special = 1 dan tanggal >= CURDATE())
    $sql_special = "SELECT * FROM `jadwal` WHERE `is_special` = 1 AND `tanggal` >= CURDATE() ORDER BY `tanggal` ASC";
    $res_special = $conn->query($sql_special);
    if ($res_special) {
        while ($row = $res_special->fetch_assoc()) {
            $upcoming_special_events[] = $row;
        }
    }

    // 2. Ambil jadwal 7 hari (Minggu Ini: Senin - Minggu)
    ...
    // 3. Ambil riwayat jadwal sebelumnya (tanggal sebelum Senin minggu ini)
...
<body class="bg-jazz-darkest text-jazz-light font-body flex flex-col min-h-screen overflow-x-hidden pt-24">
    ...
    <main class="max-w-6xl mx-auto px-6 py-12 flex-grow w-full">
        ...
        <!-- SECTION 0: EVENT SPESIAL MENDATANG -->
        <?php if (!empty($upcoming_special_events)): ?>
        <section class="mb-20">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10 border-b border-amber-500/20 pb-4">
                <div>
                    <h2 class="font-heading text-2xl md:text-3xl text-white tracking-wide">Event Spesial & Konser Eksklusif</h2>
                    <p class="text-jazz-muted text-xs md:text-sm mt-1">Pertunjukan khusus, kolaborasi eksklusif, dan bintang tamu pilihan di panggung utama</p>
                </div>
                <div class="text-xs text-amber-500 bg-amber-500/5 border border-amber-500/20 px-3 py-1.5 rounded-lg font-semibold uppercase tracking-wider">
                    Special Shows
                </div>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                <?php foreach ($upcoming_special_events as $row): 
                    $sisa_kuota = $row['kuota'] - $row['terjual'];
                    $persen_terisi = ($row['terjual'] / $row['kuota']) * 100;
                    $is_closed = (strtolower($row['status']) === 'closed');
                ?>
                    <div class="glass-card rounded-2xl p-6 flex flex-col justify-between relative overflow-hidden transition-all duration-300 special-event-glow hover:border-amber-500/30">
                        <div>
                            <!-- Header Info -->
                            ...
                            <?php if (!empty($row['special_notes'])): ?>
                                <div class="bg-amber-500/5 border border-amber-500/20 rounded-xl p-4 mb-6 text-xs text-amber-500 font-light leading-relaxed">
                            ...
                            <div>
                                ...
                                <?php elseif (!$is_logged_in): ?>
                                    <a href="login?redirect=pesan%3Fid_jadwal%3D<?php echo $row['id']; ?>" 
                                       class="block w-full bg-gradient-to-r from-amber-500 to-yellow-600 text-jazz-darkest text-center font-bold uppercase tracking-wider py-2.5 rounded-lg text-xs transform hover:-translate-y-0.5 transition-all duration-300 shadow-md shadow-amber-500/10 hover:brightness-110">
